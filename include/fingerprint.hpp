...
template <class Entry> class FingerprintIndex
{
  public:
    ...
    // featuresCompatible already overapproximates in both directions, so it covers
    // one-directional (matching) queries as well
    std::vector<Entry> matchCandidates(const Term &query) const { return candidates(query); }
