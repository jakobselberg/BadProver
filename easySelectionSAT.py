...
def collect(root: Path):
    ...
    for path in sorted(root.rglob("*.p")):
        ...
        s = status(text)
        if s is None or "Satisfiable" not in s:
            continue
...
def main():
    parser = argparse.ArgumentParser(description="copies a number of easy(rating < 0.5) sat, untyped, cnf problems from the TPTP library to a specified output directory.")

    parser.add_argument("--tptp", default=Path("inputs/TPTP-v9.2.1/Problems/"),type=Path, help="Path to the TPTP problem library.")
    parser.add_argument("--output",  default=Path("inputs/easy100SAT"),type=Path, help="Path to the output directory.")
